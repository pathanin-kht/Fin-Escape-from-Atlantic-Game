...
var item_pair_scene = preload("res://Scene/speed_item.tscn")
@onready var spawn_timer = $SpawnTimer
@onready var speed_item_sound = $SpeedItemSound

var obstacle_pair_scene = preload("res://Scene/obstaclepair.tscn")

@export var item_spawn_interval_min = 4.0
@export var item_spawn_interval_max = 6.0
...
func spawn_item():
	...
	var item_pair = item_pair_scene.instantiate() as SpeedItempair
	...
	add_child(item_pair)
	item_pair.point_scored.connect(on_point_scored_3)
...
func stop():
	spawn_timer.stop()
	for item in get_children().filter(func (child): return child is SpeedItempair):
		(item as SpeedItempair).speed = 0
		
func on_point_scored_3():
	speed_item_sound.play()
	point_scored.emit()
